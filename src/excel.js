...
export function buildWorkbook(projectData, weightUnit) {
  ...
  if (projectData.weights.length) {
    ...
    const wRows = [['Subsystem','Group',`Target Weight (${wu})`,`Estimated Weight (${wu})`,'Contingency (%)','Status','Notes']];
    projectData.weights.forEach(w => wRows.push([w.subsystem, w.group, w.target, w.estimated, w.contingency || 0, w.status, w.notes]));
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(wRows), 'Weight Budget');
  }

  // Reference Files
  if (projectData.referenceFiles && projectData.referenceFiles.length) {
    const rfRows = [['File Name','URL/Path','Generated Date','Generated By','Notes']];
    projectData.referenceFiles.forEach(f => rfRows.push([f.fileName, f.url, f.generatedDate, f.generatedBy, f.notes]));
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(rfRows), 'Reference Files');
  }
...
export function buildSampleWorkbook() {
  ...
  const wb_cols = ['Subsystem','Group','Target Weight (lb)','Estimated Weight (lb)','Contingency (%)','Status','Notes'];
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([wb_cols,
    ['Wing Assembly',            'Structures', 420, 408,  2, 'Estimated',   'T800 CFRP wing box and skins'],
    ['Fuselage',                 'Structures', 380, 392,  3, 'Over Budget', 'Heavier than target; titanium frames under review'],
    ['Tilt Mechanism',           'Structures',  95,  88,  5, 'Estimated',   'Actuators, hinges, fairings'],
    ['Landing Gear',             'Structures', 120, 115,  3, 'Estimated',   'Retractable tricycle gear'],
    ['Turbine + Gearbox',        'Propulsion', 310, 318,  2, 'Over Budget', 'Engine heavier than catalog weight due to accessories'],
    ['Battery Pack',             'Propulsion', 275, 261,  3, 'Estimated',   '85 kWh NMC; cell + module + BMS'],
    ['Rotor + Hub (×2)',         'Propulsion', 180, 176,  2, 'Estimated',   'Composite blades; titanium hub'],
    ['Flight Control Computers', 'Avionics',    45,  42,  5, 'Estimated',   'Triplex FCCs; per DO-178C DAL-A'],
    ['Sensors & Navigation',     'Avionics',    55,  58,  2, 'Over Budget', 'GNSS + INS + pitot-static; integration heavier than estimate'],
    ['HV Wiring & Bus',          'Systems',     90,  87,  3, 'Estimated',   '800V main bus; shielded cable runs'],
    ['Hydraulics',               'Systems',     65,  63,  2, 'Estimated',   'Redundant tilt actuator circuit'],
    ['Fuel System',              'Systems',     55,  52,  4, 'Estimated',   'JP-8 bladder tanks; vent and boost pump'],
    ['Furnishings & Misc',       'Other',      110, 118,  3, 'Over Budget', 'Crew seats, harness, interior; scope growth'],
  ]), 'Weight Budget');

  const rf = ['File Name','URL/Path','Generated Date','Generated By','Notes'];
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([rf,
    ['Aerodynamics Report AER-12',  'https://example.com/docs/AER-12.pdf',       '2025-05-09', 'L. Chen',             'Concept aero down-select analysis'],
    ['Power Architecture Doc PA-03', 'https://example.com/docs/PA-03.pdf',        '2025-08-01', 'T. Walsh',            'Hybrid powertrain preliminary design'],
    ['Weight Report WR-07',         'https://example.com/docs/WR-07.xlsx',       '2026-01-09', 'K. Johansson',        'Mass budget rollup at CDR'],
    ['DO-178C Compliance Matrix',   'https://example.com/docs/DO178C.pdf',       '2026-01-30', 'D. Lee',              'Flight software certification evidence'],
