...
// ─── SAVE TO EXCEL ────────────────────────────────────────────────────────────
/** Rebuilds all five sheets from current ProjectData state via SheetJS and triggers browser download. */
function saveToExcel() {
  ...
  ProjectData.tasks.forEach(t => {
    ...
    schRows.push([
      ...
      t.deps.filter(id => ProjectData.tasks.some(x => x.id === id)).join(','), t.team,
  ...
  ProjectData.specs.forEach(s => spRows.push([s.id, s.category, s.name, s.value, s.units, s.status, s.group, s.notes, s.depIds.filter(id => ProjectData.tasks.some(x => x.id === id)).join(',')]));
...
function generateSampleExcel() {
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
    ...
    ['File Administrator','ProjectData. Torres'],
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([og,
    ['ProjectData. Torres',    'Program Director',            'All Teams',   '',            'storres@tw2.aero'],
    ['R. Nakamura',  'Chief Engineer',              'All Teams',   'ProjectData. Torres',   'rnakamura@tw2.aero'],
    ...
    ['C. Martinez',  'Flight Test Director',        'Flight Test', 'ProjectData. Torres',   'cmartinez@tw2.aero'],
    ['J. Erikson',   'Chief Test Pilot',            'Flight Test', 'C. Martinez', 'jerikson@tw2.aero'],
    ['Y. Zhang',     'Certification Manager',       'Certification','ProjectData. Torres',  'yzhang@tw2.aero'],
    ['O. Brown',     'Manufacturing Lead',          'Manufacturing','ProjectData. Torres',  'obrown@tw2.aero'],
  ]), 'Org Chart');
