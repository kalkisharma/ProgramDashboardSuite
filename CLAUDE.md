...
| **v6.6.1** | Fix: editing a task (dates/notes/etc.) via the side panel re-renders the Status Report + Program Dashboard + Specs, not just the Gantt — so edits made from a Status Report row show immediately instead of after a tab switch (`saveTaskEdits`) | Done — current |
