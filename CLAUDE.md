...
`generateSampleExcel()` generates a complete **TW-2 Hybrid-Electric Tilt-Wing UAM** program workbook: 32 schedule tasks (with POC/Customer names — including a multi-name POC and a non-org "FAA ODA" customer to exercise validation), 27 specs, a 17-person org chart, a 13-subsystem weight budget, and a 5-row Reference Files sheet (with one Generated By name not in the org chart). The Project Info sheet uses the current field names (`Project Subtitle`, `File Administrator`, `Program End`, `Phase N Name` rows, `Work Days = Mon,Tue,Wed,Thu,Fri`, `Weight Unit = lb`). Use this to test all tabs. The button is hidden after any file is loaded.
...
| **v6.1.1** | **Full-team-review quick-wins.** Verified bugs: bar-drag now routes through `pushUndoSnapshot` (marks dirty → autosave + beforeunload; fixes unimported `updateUndoRedoBtns` dev-crash); free-identifier fixes (`state.dashboardLoaded`, `state.handlers.toggleHelp`, `getElementById('tooltip')`); **Program Dashboard counts leaf tasks only** (shared `leafTasks`/`isPhaseHeader` in `wbs.js`) so it agrees with Status Report/PPTX; date-picker + mini-calendar use local calendar (no UTC off-by-one). A11y: dim-mode `--muted` contrast ≥4.5:1; sub-12px floor (0.6rem collapse toggle, 8.5px org badge); Status Report rows keyboard-operable; Depth filter now applies after column sort; honest "concerns" empty state; `:focus-visible` rings on form controls; "POC Team" Gantt header + stale help copy fixed | Done — current |
