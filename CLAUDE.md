...
| `src/compute/criticalPath.js` | `computeCriticalPath(tasks, wds?)` — leaf-level CPM; work-day durations, actual-date-anchored ES, parent-dep resolution, per-component ends, completed tasks excluded (remaining path) |
...
| `src/ui/editCell.js` | `editCell(cell, {value,type,onCommit})` in-place cell editor + `wireCellEdit(cell, openPanel, edit)` — uniform single-click-opens-panel / double-click-or-Enter-edits wiring for flat tables (Specs, Weight Budget, Gantt list) |
...
- `ProjectData.weights[]` — `{ subsystem, group, target, estimated, contingency, status, notes }` — `contingency` is a maturity-based mass-growth allowance (%); predicted = estimated × (1 + contingency/100), and margin is measured against predicted
...
| Weight Budget | Optional | Subsystem, Group, Target Weight (lb), Estimated Weight (lb), Contingency (%), Status, Notes |
| Reference Files | Optional | File Name, URL/Path, Generated Date, Generated By, Notes |
| Baseline | Optional | Task ID, Baseline Start, Baseline End — frozen schedule for variance; written by "⚑ Set Baseline", parsed by `parseBaselineSheet` |
...
| `RAG At-Risk Days` / `RAG At-Risk %` / `RAG Slip Tolerance %` | Status Report at-risk thresholds (defaults 10 / 50 / 15); read by `ragConfig()` in `render/statusReport.js` |
...
| **v6.2.0** | **Onboarding.** "Load Sample Project" button parses `buildSampleWorkbook()` in-memory (one click → populated dashboard, no download/re-import round-trip); subtask-discoverability hint folded into the load toast when L3+ present; fixed `_justLoaded` ordering so load-time hint/milestone-demotion toasts actually fire (set before `parseWorkbook`); drop-zone + help copy refreshed (subtasks, POC/Customer teams, Reference Files) | Done |
| **v6.3.0** | **RAG credibility (domain review).** `ragStatus` is now **progress-aware** (earned-schedule: `expectedPct` from elapsed work-days vs actual %) and its thresholds are **configurable** via Project Info keys (`RAG At-Risk Days/%`, `RAG Slip Tolerance %`, read by `ragConfig()`); `ragStatus(t, conflictSet, today?)` takes an injectable `today` and is exported + unit-tested (`statusReport.test.js`). PPTX **Top Concerns ranked critical-path-first** (⛓ marker); legend/footnote reflect the live thresholds; help modal documents the rule; sample data seeds the keys | Done |
| **v6.4.0** | **Schedule baseline + variance** (domain review #1). "⚑ Set Baseline" (Gantt toolbar) freezes current dates into `baselineStart/End`; optional **Baseline** Excel sheet round-trips them (`parseBaselineSheet` / export). Status Report gains a **Variance** column (signed work-days vs baseline end, shown only when a baseline is set); Gantt draws a grey **baseline overlay** bar under each task. Baseline preserved through undo + draft restore. `varianceDays` unit-tested | Done |
| **v6.5.0** | **Critical-path rewrite** (domain review). `computeCriticalPath(tasks, wds)` now uses **work-day durations**, **anchors early-start to actual dates** (work-day offsets from the earliest start, so float reflects real calendar position), **resolves parent-targeted deps** to their leaf descendants, evaluates each dependency **component against its own end** (independent chains stay individually critical), and **excludes completed tasks** for a remaining-path view. Callers pass `state.ganttWorkDays`; +3 tests | Done |
| **v6.6.0** | **Performance + Weight Budget redesign** (perf/dataviz/domain review). Perf: conflict + critical-path results cached by a structural signature (zoom/theme/pan/filter reuse them); per-row `childrenOf().length` scans replaced with a precomputed parent-id Set; collapsed-parent CP highlight via O(n) ancestor-walk. Weight Budget: **fixed target-reference bars** (target tick at a constant position; bar shows over/under per row); **unified red** for over-budget across bar + KPI + margin; **contingency/MGA** column (predicted = CBE × (1+cont%), margin vs predicted) — parsed, exported, editable, shown in tooltip, seeded in sample | Done |
| **v6.6.1** | Fix: editing a task (dates/notes/etc.) via the side panel re-renders the Status Report + Program Dashboard + Specs, not just the Gantt — so edits made from a Status Report row show immediately instead of after a tab switch (`saveTaskEdits`) | Done |
| **v6.7.0** | At-risk (RAG) thresholds editable in the **Project Info panel** — dedicated "Status (RAG) Thresholds" section (number inputs for At-Risk window / completion % / slip tolerance, pre-filled from `ragConfig()` with helper text); saving re-renders the Status Report. Writes the `RAG At-Risk Days/%`, `RAG Slip Tolerance %` info keys (round-trip in Excel) | Done |
| **v6.7.1** | Docs/help refresh for the v6.1–6.7 work: in-app help modal (Status Report Variance + RAG-threshold panel editing, Gantt Set Baseline + critical path, Weight Budget contingency, Baseline/Contingency in the Excel-sheets list) + README (Baseline/Weight sheets, RAG/Weight-Unit Project Info keys, Set-Baseline/RAG-threshold actions) + TOUR module map (hierarchy.js, CPM/WBS descriptions) | Done |
| **v6.7.2** | Fix the over-eager tab-switch guard: switching tabs with a side-panel edit form open now only prompts when the form has **actually been changed** (`state.spFormDirty`, set on `input`); a pristine panel switches freely and just closes. Clearer "Discard… and switch tabs?" confirm. Also: `parseInfoSheet` skips the `Field/Value` header row (no junk `Field` info entry) | Done |
| **v6.8.0** | **Uniform inline editing** (team review). New `ui/editCell.js` (`editCell` + `wireCellEdit`): **single-click opens the row's panel, double-click / Enter edits a cell in place** (200 ms disambiguation only on editable cells; hover ✎ affordance; blur-commits; `pushUndo` + undo toast). Applied to **Gantt** name/% (changed from single-click → double-click), **Vehicle Specs** (Name/Value/Units/Notes; status still click-to-cycle), and **Weight Budget** (Estimated/Target). Status Report + Org Chart deliberately stay panel-only; derived/rolled-up cells stay read-only. Ctrl/modifier-click rejected (macOS context-menu + a11y). Docs/help updated | Done |
| **v6.8.1** | Docs: new user-facing `CHANGELOG.md` (complete version history, v1.22.0–v6.8.1, backfilled & grouped by series), linked from README. No app changes | Done — current |
