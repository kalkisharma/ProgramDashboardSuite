...
| **v6.7.0** | At-risk (RAG) thresholds editable in the **Project Info panel** — dedicated "Status (RAG) Thresholds" section (number inputs for At-Risk window / completion % / slip tolerance, pre-filled from `ragConfig()` with helper text); saving re-renders the Status Report. Writes the `RAG At-Risk Days/%`, `RAG Slip Tolerance %` info keys (round-trip in Excel) | Done — current |
