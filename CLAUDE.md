...
| **v6.1.0** | Status Report **PowerPoint style/content overhaul** (expert review): slide 1 redesigned — executive summary line, KPI **cards** (rounded shapes), and a **Top Concerns** table (replaces the slide-2-duplicate phase table); slide 2 gains a Program **totals row**; slide 3 task table **indents the WBS hierarchy** (bold parents, milestone ◆), adds a **RAG legend** + at-risk-rule + "⚠ = name not in org chart" key, carries org-validation markers; WCAG contrast fixes (no more `#9CA3AF` body text), unified %-color (no categorical blue), "Status as of" header, word-boundary truncation, baseline-slip note (`originalTasks`) | Done — current |
